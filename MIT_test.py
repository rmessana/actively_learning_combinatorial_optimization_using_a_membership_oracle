...
from MIT_optimizer import MITOptimizer
from oracle import Oracle
from separator import SEPLinearSeparator, SVMLinearSeparator
from MIT_strategy import MITCUTStrategy, MITSIMStrategy
...
for separator_name, strategy_name in algorithms:
    ...
    output_directory = "MIT_output/logs/{}+{}".format(separator_name, strategy_name)
    ...
    for instance_number in instance_numbers:
        ...
        with open("data/MIT_instances/{}.pickle".format(instance_number), "br") as f:
            student_instance = pickle.load(f)
        values = student_instance["values"]
        weights = student_instance["weights"] / student_instance["capacity"]
        capacity = 1.0

        units = np.array([sum(eval(u)) for u in df.units])
        prerequisites = [eval(p) for p in df.prerequisites]
        corequisites = [eval(c) for c in df.corequisites]
        alternatives = [eval(a) for a in df.alternatives]

        model = gp.Model("model")
        variables = model.addVars(range(dimension), vtype=gp.GRB.BINARY)
        model.addConstr(gp.quicksum(weights[j] * variables[j] for j in range(dimension)) <= capacity)
        for j, p in enumerate(prerequisites):
            for code_list in p:
                model.addConstr(variables[j] <= gp.quicksum(variables[code] for code in code_list))
        for j, c in enumerate(corequisites):
            for code_list in c:
                model.addConstr(variables[j] <= gp.quicksum(variables[code] for code in code_list))
        ...
        optimizer = MITOptimizer()

        # strategy -----------------------------------------------------

        if strategy_name == "CUT":
            strategy = MITCUTStrategy(dimension=dimension)
        elif strategy_name == "SIM":
            strategy = MITSIMStrategy(dimension=dimension)
        else:
            strategy = None
        ...
        with (open("{}/{}.log".format(output_directory, instance_number), 'w') as f):
            ...
            log(f, "Separator: {}, strategy: {}, dimension: {}, instance number: {}".format(
                    ...
                    if oracle.n_queries in n_query_checkpoints:
                        ...
                        checkpoint_results[oracle.n_queries].append(
                            {"instance_number": instance_number,
                             ...
                             "n_infeasible": points["infeasible"].shape[0] + new_points["infeasible"].shape[0],
            ...
            log(f, str(df.iloc[np.where(best_point == 1)]))
