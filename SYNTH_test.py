...
from SYNTH_optimizer import SYNTHOptimizer
from oracle import Oracle
from separator import SEPLinearSeparator, SVMLinearSeparator
from SYNTH_strategy import SYNTHCUTStrategy, SYNTHSIMStrategy
...
for separator_name, strategy_name in algorithms:
    ...
    output_directory = "SYNTH_output/logs/{}+{}".format(separator_name, strategy_name)
    ...
    for instance_number in instance_numbers:
        ...
        with open("data/SYNTH_instances/{}_{}.pickle".format(dimension, instance_number), "br") as f:
            synthetic_instance = pickle.load(f)
        values = synthetic_instance["values"]
        weights = synthetic_instance["weights"]
        capacity = synthetic_instance["capacity"]
        dependencies = synthetic_instance["dependencies"]

        model = gp.Model("model")
        variables = model.addVars(range(dimension), vtype=gp.GRB.BINARY)
        model.addConstr(gp.quicksum(weights[j] * variables[j]
                                    for j in range(dimension)) <= capacity)
        for i, j in dependencies:
            model.addConstr(variables[i] >= variables[j])
        ...
        points = {"feasible": np.array([np.zeros(dimension)]),
                  "infeasible": np.array([np.ones(dimension)])}
        new_points = {"feasible": np.empty((0, dimension)), "infeasible": np.empty((0, dimension))}

        best_point = points["feasible"][np.argmax([np.dot(values, point)
        ...
        if strategy_name == "CUT":
            strategy = SYNTHCUTStrategy(dimension=dimension)
        elif strategy_name == "SIM":
            strategy = SYNTHSIMStrategy(dimension=dimension)
        else:
            strategy = None
        ...
        status = {"dependencies": dependencies, "values": values, "b": b, "w": w, "points": points,
                  "best_point": best_point}

        with (open("{}/{}.log".format(output_directory, instance_number), 'w') as f):

            log(f,
                "Separator: {}, strategy: {}, dimension: {}, instance number: {}".format(
                    separator_name, strategy_name, dimension, instance_number))

            log(f, "\nExact optimal solution: {}".format(
                ''.join([str(round(n)) for n in exact_optimal_solution])))
            ...
            while True:
                ...
                new_points = {"feasible": np.empty((0, dimension)),
                              "infeasible": np.empty((0, dimension))}
                ...
                status = {"dependencies": dependencies, "values": values, "b": b, "w": w,
                          "points": points, "best_point": best_point}
                ...
                if point is None:
                    ...
                    if label == 1:
                        new_points["feasible"] = np.concatenate(
                            [new_points["feasible"], [point]], axis=0)
                    ...
                    log(f, "{}\t\t\t{}\t{}\t{}\t(best objective: {}) {} (elapsed time: {})".format(
                        iteration, ''.join([str(round(n)) for n in point]),
                        round(objective_value, 4), {1: "feas", 0: "infeas"}[label],
                        best_objective_value, message, strategy_elapsed_time))

                    if (exact_objective_value == best_objective_value or
                            oracle.n_queries >= max_n_queries):
                        break
                ...
                if point is None:
                    ...
                    if best_point.sum() == 0:
                        selective_weight_distance = float(dimension)
                    else:
                        selective_weight_distance = np.linalg.norm(
                            (weights / capacity + w / b) * best_point, ord=1) / best_point.sum()
                    log(f, "{}\t\t\tinfo\t(selective weight distance: {})".format(
                        iteration, selective_weight_distance))
                    ...
                    log(f, "{}\t\t\t{}\t{}\t{}\t(best objective: {}) {} (elapsed time: {})".format(
                        iteration, ''.join([str(round(n)) for n in point]),
                        round(objective_value, 4), {1: "feas", 0: "infeas"}[label],
                        best_objective_value, message, optimization_elapsed_time))

                    if (exact_objective_value == best_objective_value
                            or oracle.n_queries >= max_n_queries):
                        break
                ...
                points["feasible"] = np.concatenate(
                    [points["feasible"], new_points["feasible"]], axis=0)
                new_points["infeasible"] = np.unique(new_points["infeasible"], axis=0)
                points["infeasible"] = np.concatenate(
                    [points["infeasible"], new_points["infeasible"]], axis=0)
            ...
            if exact_objective_value == best_objective_value:
                ...
                log(f, "\nSolved in {} queries and {} iterations".format(oracle.n_queries,
                                                                         iteration))
                instance_solved_flag = True

            else:

                log(f, "\nUnsolved after {} queries and {} iterations".format(oracle.n_queries,
                                                                              iteration))
            ...
            log(f, "Separation cumulative elapsed time: {}".format(
                separation_cumulative_elapsed_time))
            log(f, "Strategy cumulative elapsed time: {}".format(strategy_cumulative_elapsed_time))
            log(f, "Optimization cumulative elapsed time: {}".format(
                optimization_cumulative_elapsed_time))
